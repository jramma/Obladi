...
export default function ObjectList({ obj, objectKey }: any) {
  ...
  return (
    <Link
      href={"/profile/objects/" + obj._id}
      key={objectKey}
      className="no-underline-effect flex flex-col gap-2 mx-2 bg-black/10 dark:bg-white/10 p-2 rounded-lg hover:bg-black/30 hover:dark:bg-white/30 transition-all duration-200"
    >
      ...
      <div className="flex gap-2 mt-2 flex-wrap">
        {obj.tags.length > 1 &&
          obj.tags.map((tag: string, i: number) => (
            <span
              key={i}
              className="bg-tertiary text-black text-sm px-2 py-1 rounded-full"
            >
              #{tag}
            </span>
          ))}
